import {mu, asyr2kms} from "../modules/Constants";
...
chart.create = function(el, orbit) {
  ...
  chart.svg = {
    line: d3.svg.line()
      ...
      .interpolate("basis"),
    axis: {
      x: d3.svg.axis().scale(chart.scale.x).orient("bottom").ticks(5).outerTickSize(0).tickFormat(d3.format(".0f")),
      y: d3.svg.axis().scale(chart.scale.y).orient("left").ticks(5).outerTickSize(0).tickFormat(d3.format(".0f"))
    }
  };
...
chart.update = function(el, orbit) {
  ...
  var t0 = orbit.t0 - P/2, dt = P / 100,
      ...
      points = time.map(t => {
        ...
        return [t, asyr2kms(p.vz)];
      });

  chart.scale.x.domain(d3.extent(points, d => d[0]));
  chart.scale.y.domain(d3.extent(points, d => d[1]));
  ...
  svg.select(".axis.y")
    .attr("transform", `translate(${chart.scale.x(orbit.t0)},0)`)
